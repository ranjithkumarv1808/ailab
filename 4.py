...
def evaluate(board):
 ...
 for col in range(3):
   ...
   if board[0][2]==board[0][2]==board[1][1]==board[2][0]!=EMPTY:
  ...
  for row in range(3):
    for col in range(3):
      if board[row][col]==EMPTY:
         ...
         board[row][col]=EMPTY
         best_score=max(best_score, current_score) if is_maximixing else min(best_score, current_score)
  return best_score
#Find the best move
